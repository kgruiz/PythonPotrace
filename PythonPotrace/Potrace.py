...
from .utils import (
    sign, mod, xprod, dpara, ddenom, interval, iprod1, between, ddist, cprod, iprod,
    render_curve, luminance
)
...
class Potrace:
    ...
    def _validateParameters(self, params):
        ...
        # Validate threshold
        if params and 'threshold' in params and params['threshold'] != Potrace.THRESHOLD_AUTO:
    ...
    def _processLoadedImage(self, pil_image):
        ...
        for y in range(h):
            for x in range(w):
                r, g, b, *rest = pixels[x, y] if len(pixels[x, y]) >= 3 else (255, 255, 255, 255)
    ...
    def loadImage(self, target, callback=None):
        """
    ...
    def _bmToPathlist(self):
        ...
        blackMap = self._luminanceData.copy(
            lambda lum, idx:
                0 if (blackOnWhite and lum > threshold) or
                     (not blackOnWhite and lum < threshold)
                else 1
        )
        ...
        def majority(x, y):
            ...
            for i in range(2, 5):
                ...
                for a in range(-i+1, i):
                    ct += blackMap.getValueAt(x + a, y + i - 1)
                    ct -= (1 - blackMap.getValueAt(x + a, y + i - 1))
                    ct += blackMap.getValueAt(x + i - 1, y + a - 1)
                    ct -= (1 - blackMap.getValueAt(x + i - 1, y + a - 1))
                    ct += blackMap.getValueAt(x + a - 1, y - i)
                    ct -= (1 - blackMap.getValueAt(x + a - 1, y - i))
                    ct += blackMap.getValueAt(x - i, y + a)
                    ct -= (1 - blackMap.getValueAt(x - i, y + a))
                if ct > 0:
                    return 1
                elif ct < 0:
                    return 0
        ...
        def findPath(x_start, y_start):
            ...
            # initial direction
            dirx, diry = 0, 1
            p.sign = '+' if blackMap.getValueAt(x, y) == 1 else '-'
            while True:
                ...
                # left & right checks
                l = blackMap.getValueAt(x + (dirx + diry - 1)//2,
                                        y + (diry - dirx - 1)//2)
                r = blackMap.getValueAt(x + (dirx - diry - 1)//2,
                                        y + (diry + dirx - 1)//2)
                ...
                if r == 1 and l == 0:
                    ...
                    if (turnPolicy == 'right' or
                        (turnPolicy == 'black' and p.sign == '+') or
                        (turnPolicy == 'white' and p.sign == '-') or
                        (turnPolicy == 'majority' and majority(x, y) == 1) or
                        (turnPolicy == 'minority' and majority(x, y) == 0)):
                        tmp = dirx
                        dirx = -diry
                        diry = tmp
                    else:
    ...
    def _processPath(self):
        ...
        def calcLon(path):
            ...
            for i in reversed(range(n)):
                ct = [0, 0, 0, 0]
                dir_idx = (3 + 3*(pt[(i+1)%n].x - pt[i].x) + (pt[(i+1)%n].y - pt[i].y))//2
                ct[dir_idx] += 1
                ...
                foundk = False
                while True:
                    dir_idx = (3 + 3*sign(pt[k].x - pt[k1].x) + sign(pt[k].y - pt[k1].y))//2
                    ct[dir_idx] += 1
            ...
            for i in range(n-2, -1, -1):
                if (j >= i+1 and j <= pivk[i]) or (pivk[i] < i+1 and (j >= i+1 or j <= pivk[i])):
                    j = pivk[i]
                lon[i] = j
        ...
        def bestPolygon(path):
            ...
            for j in range(1,m+1):
                ...
                for i in range(seg1[j], seg0[j]+1):
                    ...
                    if length_[j]> length_[j-1] + 1 or (length_[j] == length_[j-1]+1 and pen[j]> thispen):
                        pen[j] = thispen
                        prev[j] = i
                        length_[j] = length_[j-1] +1
            path.m = m
    ...
    def getSVG(self):
        ...
        width = self._params['width'] if self._params['width'] else self._luminanceData.width
        height = self._params['height'] if self._params['height'] else self._luminanceData.height
        scale = {
            'x': width / self._luminanceData.width if self._luminanceData.width else 1,
            'y': height / self._luminanceData.height if self._luminanceData.height else 1
        }
